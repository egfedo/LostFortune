...
int FieldBuilder::buildField(Field &templt, LineStream* ptr) {
    ...
    SpikeEvent spike(5);
    HealEvent heal(10, 2);
    ChestEvent bombs(Player::consumeID::bomb, 3);
    ChestEvent coin(Player::consumeID::coins, 1);
        ...
        for (size_t j = 0; j < field.getHeight(); j++) {
            ...
            switch (line[j]) {
                ...
                case 'H':
                    ...
                    break;
                case 'B':
                    eventSample.setEvent(std::make_shared<ChestEvent>(bombs));
                    field.setTile(i, j, eventSample);
                    break;
                case 'C':
                    eventSample.setEvent(std::make_shared<ChestEvent>(coin));
                    field.setTile(i, j, eventSample);
                    break;
                case 'P':
