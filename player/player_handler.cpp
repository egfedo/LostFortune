...
PlayerHandler::PlayerHandler(size_t x, size_t y, Player &instance, Field& field) :
    x(x), y(y), player(std::make_shared<Player>(instance)), field(std::make_shared<Field>(field)) {}
...
std::shared_ptr<Field> PlayerHandler::getField() {
    return field;
}

PlayerHandler::PlayerHandler(const PlayerHandler &handler) {
    x = handler.x;
    y = handler.y;
